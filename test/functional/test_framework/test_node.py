...
class TestNode():
    ...
    def __init__(self, i, datadir, *, chain, rpchost, timewait, timeout_factor, bitcoind, bitcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        ...
        self.bitcoinconf = os.path.join(self.datadir, "oblivion.conf")
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'obliviond exited with status {} during initialization'.format(self.process.returncode)))
            try:
                rpc = get_rpc_proxy(
        ...
        self._raise_assertion_error("Unable to connect to obliviond after {}s".format(self.rpc_timeout))
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into obliviond")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to obliviond
        expected_msg: regex that stderr should match when obliviond fails

        Will throw if obliviond starts without an error.
        Will throw if an expected_msg is provided and it does not match obliviond's stdout."""
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('obliviond failed to start: %s', e)
                self.running = False
                self.process = None
...
class TestNodeCLI():
    ...
    def send_cli(self, command=None, *args, **kwargs):
        ...
        self.log.debug("Running oblivion-cli {}".format(p_args[2:]))
