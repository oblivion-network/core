    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional OBLV outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes OBLV and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::OBLV_TO_OBLV) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_oblv);
        }
    } else {
        // First try to construct a OBLV-to-OBLV transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::OBLV_ONLY;
        ...
        // If OBLV-to-OBLV fails, create a peg-out transaction
