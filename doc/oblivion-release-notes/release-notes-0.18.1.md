Oblivion Core version 0.18.1 is now available from:

  <https://download.oblivion.org/oblivion-0.18.1/>
...
`/Applications/Oblivion-Qt` (on Mac) or `obliviond`/`oblivion-qt` (on
Linux).
...
Oblivion Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Oblivion Core on unsupported systems.
...
10.10. Additionally, Oblivion Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:19327:19327` (this is an extra `:19327` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@oblivion.org) the Oblivion Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 OBLV),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in OBLV per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/oblivion-conf.md)
  about the `oblivion.conf` file describes how to use it to configure
  Oblivion Core.

- A new document introduces Oblivion Core's BIP174 [Partially-Signed
  Oblivion Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Oblivion-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Oblivion Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Oblivion Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Oblivion Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart obliviond with the `-deprecatedrpc=generate`
...
- A new `oblivion-wallet` tool is now distributed alongside Oblivion
...
This section describes planned changes to Oblivion Core that may affect
other Oblivion software and services.

- Since version 0.16.0, Oblivion Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Oblivion Core 0.20 (expected about
  a year after 0.18), Oblivion Core will default to native segwit
  ...
  to bech32 addresses, and if the Oblivion Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Oblivion Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Oblivion Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Oblivion Core
...
- A sub-project of Oblivion Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Oblivion Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Oblivion Core's own implementation, although entropy
  gathered by Oblivion Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Oblivion Core a
...
- On macOS, Oblivion Core now opts out of application CPU throttling
...
- #19327 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
