...
The Oblivion Core daemon can be configured to act as a trusted "border
router", implementing the oblivion wire protocol and relay, making
...
The ZeroMQ feature in Oblivion Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building obliviond:
...
For instance:
    ...
    $ obliviond -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/obliviond.tx.raw \
...
These options can also be provided in oblivion.conf.
...
From the perspective of obliviond, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into obliviond directly. Furthermore, no information is
...
using. Obliviond appends an up-counting sequence number to each
